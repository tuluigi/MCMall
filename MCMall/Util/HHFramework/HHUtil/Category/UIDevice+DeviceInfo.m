...
@end
@implementation UIDevice (HHFuncation)

+(BOOL)enableCallUpAndSMS{
...
+(void)callUp:(NSString *)tel{
    if (![UIDevice enableCallUpAndSMS]) {
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"该设备不支持拨号功能" delegate:nil cancelButtonTitle:@"好的,知道了" otherButtonTitles:nil,nil];
        [alert show];
    }else{
...
+(BOOL)callUpNoneCallBack:(NSString *)tel{
    if (![UIDevice enableCallUpAndSMS]) {
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"该设备不支持拨号功能" delegate:nil cancelButtonTitle:@"好的,知道了" otherButtonTitles:nil,nil];
        [alert show];
        return NO;
    }else{
        return   [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[@"tel://" stringByAppendingString:tel]]];
    }
}
+(BOOL)sendSMS:(NSString *)tel{
    if (![UIDevice enableCallUpAndSMS]) {
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"该设备不支持拨号功能" delegate:nil cancelButtonTitle:@"好的,知道了" otherButtonTitles:nil,nil];
        [alert show];
        return NO;
    }else{
        return   [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[@"sms://" stringByAppendingString:tel]]];
    }
...
+(BOOL)sendEmail:(NSString *)email{
    return  [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[@"mailto://" stringByAppendingString:email]]];
}
+(BOOL)openSafari:(NSString *)url{
    if (url&&url.length) {
        return  [[UIApplication sharedApplication] openURL:[NSURL URLWithString:url]];
    }else{
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"打开浏览器失败" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil,nil];
        [alert show];
        return NO;
    }
}
