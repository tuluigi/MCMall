...
@interface UIDevice (HHFuncation)
+(BOOL)enableCallUpAndSMS;//是否支持电话和短信
+(void)callUp:(NSString *)tel;//打电话;默认带有电话结束回调到当前的app
+(BOOL)callUpNoneCallBack:(NSString *)tel;//没有回调的打电话
+(BOOL)sendSMS:(NSString *)tel;// 发短信
+(BOOL)sendEmail:(NSString *)email;// 发邮件
+(BOOL)openSafari:(NSString *)url;//打开浏览器
@end
